...
> **UPDATE (post-merge): `--setting-sources` removed; projection is now symlink-only / opt-in import.** The `--setting-sources project,local` flag below turned out to be **incompatible with some `claude` installations**, so it was removed from the writing spawns. With no flag available to exclude the user-level `~/.claude/CLAUDE.md`, we no longer auto-write the `@USER.md` import either: `syncToClaudeDir()` now **only symlinks `~/.claude/USER.md`** (which Claude Code does NOT auto-load) and returns the `@USER.md` line for the user to add to `~/.claude/CLAUDE.md` **themselves**. So Decision A (auto-import) became **Decision B (opt-in import)**, and the profile still enters doc-editor's prompts only via `memory.compose()`. The rest of this section is kept as the historical record.

**Fix (mechanism corrected after a smoke test — `--bare` was the original plan but BREAKS subscription auth).** Empirically (Claude Code 2.1.168): `--bare` "skips … keychain reads … OAuth and keychain are never read", so a subscription user (`apiKeySource: none`) gets *"Not logged in · Please run /login"*. `--bare` is therefore unusable here. The working lever is **`--setting-sources project,local`** on writing spawns: it loads only the project+local setting/memory sources and **excludes `user`** — so `~/.claude/CLAUDE.md` (and its `@USER.md`) is not auto-loaded — while **auth (keychain) is untouched**. Verified end-to-end: with it, a cwd `CLAUDE.md` token is suppressed when its source is excluded, auth still works, and **Read + `--add-dir` still work** (attachments unaffected). Memory then enters doc-editor **only** through the controlled `memory.compose()` injection (lean, retrieval-scoped, guardrailed — Q5/§5.2). This *strengthens* the existing `RUN_DIR` protection (which only ever blocked the *project* CLAUDE.md).

- **Apply `--setting-sources project,local` to:** generate, revise, analyze, distill, interview, compileBrief, toDeck (voice is injected explicitly, so nothing is lost). Minor effect: the user's *configured* default model/effort no longer applies when the request omits them (the app already passes the picker's values, empty = CLI default), and other user-level settings are skipped for these calls — acceptable, and more predictable.
- **Do NOT add it to mail spawns** — they depend on user-level MCP configuration. Consequence: once `~/.claude/CLAUDE.md` exists, mail calls *will* auto-load the profile (extra cost; guardrail not applied to drafted mail). Flagged as a v1 follow-up (§11.6).
- **Verified (smoke test, this build):** `--bare` breaks auth (rejected); `--setting-sources project,local` preserves subscription auth, Read, and `--add-dir`, and `--setting-sources` gates CLAUDE.md memory loading.

Seeding is **deferred** (Q6): the *productized* onboarding/import for general users stays out of v1 and needs more thought. The **developer's own instance is hand-seeded once, out-of-band** (exactly the "out-of-band" path the spec allows) — a manual OpenClaw → canonical-store curation that populates `~/.config/doc-editor/memory/` only. It does **not** set up the `~/.claude/` projection (that waits for the `--setting-sources project,local` switch, §2.2), so the seed files are inert until `lib/memory.js` reads them. No write-back to OpenClaw, ever.
...
- **`lib/claude.js`.** `generate()` appends the verbatim intake block + `memory.compose(...)`; `revise()` appends `contextSummary` + `memory.compose(...)`; new `distillContext(intake)`; reuse `analyze()` for the capture pass. Keep all spawns in `RUN_DIR`, and **add `--setting-sources project,local` to the writing spawns** (generate/revise/analyze/distill/interview/compileBrief/toDeck) so they don't auto-load `~/.claude/CLAUDE.md`+`USER.md` (§2.2) — memory enters only via `memory.compose()`. Mail spawns keep all sources (§11.6).
...
2. **`lib/memory.js` store + manifest + projection** (symlink/copy + the `@USER.md` import), with `USER.md` scaffolding and atomic writes. **Couple this with the `--setting-sources project,local` switch on writing spawns** (§2.2; `--bare` was rejected — it breaks subscription auth) — the two must land together, since creating `~/.claude/CLAUDE.md` is what makes the switch necessary. Smoke-verified already this build.
...
6. **Mail + memory. (New, flagged — v1 follow-up.)** Mail spawns keep all setting sources (they rely on user-level MCP config), so once `~/.claude/CLAUDE.md` exists mail calls will auto-load the full profile (extra cost; the leakage guardrail isn't applied to drafted mail). Resolve later: exclude `user` for mail too while passing MCP via `--mcp-config`/`--strict-mcp-config`, or accept profile grounding for mail and apply the guardrail explicitly there.
