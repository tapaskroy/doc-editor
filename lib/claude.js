...
function runJsonTurn(systemPrompt, prompt, { model, effort, web } = {}) {
...
async function revise({ markdown, comments = [], instruction = '', history = [], model, effort, web }) {
  ...
  let raw = await runJsonTurn(system, prompt, opts);
  ...
  if (!parsed) {
    ...
    raw = await runJsonTurn(system, sterner, opts);
...
async function toDeck(markdown, { model, effort } = {}) {
  ...
  let raw = await runJsonTurn(DECK_SYSTEM + NO_TOOLS_NOTE, prompt, opts);
  let deck = firstJsonObject(raw);

  if (!deck || !Array.isArray(deck.slides)) {
    const sterner = prompt + '\n\nReply with ONLY the JSON object described — no prose, no code fence.';
    raw = await runJsonTurn(DECK_SYSTEM + NO_TOOLS_NOTE, sterner, opts);
