...
function spawnClaude(args, prompt, { addDir } = {}) {
  ...
  // Load only project+local setting sources, never `user`. RUN_DIR already avoids the
  // PROJECT CLAUDE.md; this additionally keeps the USER-level ~/.claude/CLAUDE.md (and
  // its @USER.md personal-memory import) out of every writing call — so the profile
  // enters the prompt ONLY through the guardrailed memory.compose() path, not as an
  // un-scoped auto-load. (`--bare` would also suppress it but it skips keychain reads
  // and breaks subscription auth; --setting-sources leaves auth untouched. Verified.)
  const finalArgs = ['-p', '--setting-sources', 'project,local', ...args, ...(addDir ? ['--add-dir', addDir] : [])];
