...
// Email writing mode. An email reuses the document pipeline (streaming, versions,
// title) but reads as an email, not a report: the single H1 is the SUBJECT line,
// and everything after it is the body. The composer presents that H1 as the
// editable subject; at commit the app peels it off as the envelope subject.
const EMAIL_GENERATE_SYSTEM = `You are an expert email-writing engine. Produce ONE email in GitHub-flavored Markdown:
- Begin with a single H1 that is the SUBJECT line (# Subject) — concise and specific, no trailing punctuation.
- After it, write the email body: a fitting greeting, short scannable paragraphs, and a sign-off. Use lists only when they genuinely help; keep headings rare (most emails need none).
- Write in an email register — direct, courteous, easy to skim. Prefer short paragraphs over walls of text. Match length to the purpose.

Do NOT write To/Cc/Bcc, a "Subject:" label, or any other envelope metadata — only the H1 subject and the body. Do NOT add preamble or commentary, and do NOT wrap the email in a code fence.`;
...
// Baseline anti-"AI tic" guidance, applied to all writing/editing regardless of
// the chosen voice. The em dash ban is a hard rule (per user preference).
const ANTI_TIC_NOTE =
  '\n\nWrite in clean, natural prose and avoid AI writing tics:\n' +
  '- Do NOT use em dashes (—) or en dashes (–) at all. Use commas, parentheses, a colon, or two sentences instead.\n' +
  '- No throat-clearing or filler ("It\'s important to note", "In conclusion", "Overall", "In today\'s world").\n' +
  '- Avoid the "not just X, but Y" / "isn\'t merely … it\'s …" construction.\n' +
  '- Avoid overused buzzwords: delve, tapestry, testament, realm, landscape, underscore, boast, foster, leverage, navigate.\n' +
  '- Be direct and concrete; do not over-hedge or pad.';
...
// Whether the installed `claude` CLI advertises a model alias (it lists them in
// `claude --help`, e.g. "fable"). Used to hide a model the CLI can't run so the
// picker can't offer something that would error on draft. Pure + testable.
function helpMentionsModel(helpText, alias) {
  const a = String(alias || '').replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
  return !!a && new RegExp(`\\b${a}\\b`, 'i').test(String(helpText || ''));
}

// Async, cached: read `claude --help` once and check for the alias. Best-effort —
// if --help can't be read (no CLI), treat the alias as unsupported.
let _helpText = null;
function modelAliasSupported(alias) {
  return new Promise((resolve) => {
    if (_helpText != null) return resolve(helpMentionsModel(_helpText, alias));
    let out = '';
    let child;
    try { child = spawn('claude', ['--help']); }
    catch { _helpText = ''; return resolve(false); }
    child.stdout.on('data', (d) => (out += d.toString()));
    child.stderr.on('data', (d) => (out += d.toString()));
    child.on('error', () => { _helpText = ''; resolve(false); });
    child.on('close', () => { _helpText = out; resolve(helpMentionsModel(out, alias)); });
  });
}
...
function spawnClaude(args, prompt, { addDir } = {}) {
  // Re-ensure the run dir on every spawn. It lives under the OS temp dir, which can
  // be purged out from under a long-running server (macOS cleans /var/folders and
  // /tmp). Creating it once at module load is not enough: if it later disappears, a
  // missing cwd makes spawn throw ENOENT, which surfaces as "Failed to launch claude".
  try { fs.mkdirSync(RUN_DIR, { recursive: true }); } catch { /* best effort */ }
  // NOTE: we previously passed `--setting-sources project,local` here to keep the
  // user-level ~/.claude/CLAUDE.md out of writing calls, but that flag is incompatible
  // with some `claude` installations, so it's removed. RUN_DIR still keeps the PROJECT
  // CLAUDE.md out. To avoid the USER-level CLAUDE.md auto-loading the profile into our
  // calls (cost + a leakage-guardrail bypass), the ~/.claude projection no longer
  // writes an `@USER.md` import (lib/memory.syncToClaudeDir) — it only symlinks
  // USER.md, which Claude Code does NOT auto-load. So the profile still enters our
  // prompts ONLY through the guardrailed memory.compose() path.
...
// The model that actually did the work. `modelUsage` can list MORE than one
// model — Claude Code runs a small Haiku side-call alongside the main model on
// every generation — so we must NOT just take the first key (that mislabels an
// Opus job as Haiku). Pick the dominant model by cost.
function primaryModel(o) {
  const mu = o && o.modelUsage;
  if (mu && typeof mu === 'object') {
    const entries = Object.entries(mu);
    if (entries.length) {
      return entries.reduce((best, e) => ((e[1] && e[1].costUSD) || 0) > ((best[1] && best[1].costUSD) || 0) ? e : best)[0];
    }
  }
...
function generate(premise, { onDelta, onDone, onError, onReset, model, effort, web, style, references, addDir, kind, intake, memory, guardrails }) {
  const hasRefs = !!(references && references.trim());
  const hasIntake = Array.isArray(intake) && intake.length > 0;
  const base = kind === 'email' ? EMAIL_GENERATE_SYSTEM : GENERATE_SYSTEM;
  // `memory` is the personal-memory block from lib/memory.compose() — standing facts
  // about the user, carrying the grounding/don't-volunteer guardrail. `guardrails` is
  // lib/feedback.compose() — kept "avoid these" corrections. Both live in the system
  // prompt (like the voice guide), not the user prompt.
  const system =
    base + ANTI_TIC_NOTE + (guardrails || '') + (web ? WEB_TOOLS_NOTE : NO_TOOLS_NOTE) + styleNote(style) + (memory || '') + (hasRefs ? ATTACH_NOTE : '');
  // Carry the planning conversation verbatim into the FIRST draft (hybrid C): this
  // is a streaming prose call with no JSON-mode risk, so the model gets the user's
  // exact words and can't re-lose the specifics the brief compressed away (the Bali
  // bug). Revisions use a distilled summary instead, never this raw transcript.
  const convo = hasIntake
    ? '----- CONVERSATION THAT SHAPED THIS DOCUMENT -----\n' +
      'The user planned this document in the conversation below. Honor every concrete ' +
      'detail they gave (names, who/what/when/where, numbers, specifics); do not invent ' +
      'facts they did not state, and do not omit the ones they did.\n\n' +
      renderTranscript(intake)
    : '';
  const ctx = [hasRefs ? references : '', convo].filter(Boolean).join('\n\n');
...
async function revise({ markdown, comments = [], instruction = '', history = [], contextSummary = '', memory = '', guardrails = '', model, effort, web, style, references, addDir }) {
  ...
  // Distilled background from the planning conversation (hybrid C): grounding only,
  // NEVER the raw transcript (conversational text breaks clean JSON-edit mode).
  if (contextSummary && contextSummary.trim()) {
    parts.push(`BACKGROUND (durable facts from the planning conversation — for grounding only; already reflected in the document, do not re-add or restate):\n${contextSummary.trim()}\n`);
  }
  ...
  const system =
    REVISE_SYSTEM + ANTI_TIC_NOTE + (guardrails || '') + (web ? WEB_TOOLS_NOTE : NO_TOOLS_NOTE) + styleNote(style) + (memory || '') + (hasRefs ? ATTACH_NOTE : '');
...
async function toDeck(markdown, { model, effort } = {}) {
  ...
  let r = await runTurn(DECK_SYSTEM + ANTI_TIC_NOTE + NO_TOOLS_NOTE, prompt, opts);
  let usage = r.usage;
  ...
  if (!deck || !Array.isArray(deck.slides)) {
    ...
    r = await runTurn(DECK_SYSTEM + ANTI_TIC_NOTE + NO_TOOLS_NOTE, sterner, opts);
    usage = sumUsage(usage, r.usage);
...
const DISTILL_SYSTEM = `You distill a planning conversation into a compact CONTEXT NOTE for an editor who will make small, surgical edits to the resulting document.

Capture every concrete, document-relevant fact the user supplied: names, who/what/when/where, numbers, specific details, decisions, and constraints. Preserve specifics verbatim (exact names, numbers, places). Drop the conversational back-and-forth, pleasantries, and any meta-discussion about the writing process itself.

Respond with ONLY a JSON object (no prose, no code fence): {"summary":"…"}

The summary is factual background, NOT instructions to rewrite. Keep it terse — a short paragraph or a few lines. If the conversation holds no durable facts, return {"summary":""}.`;

// Distill the raw intake transcript into a compact, fact-preserving context note
// for the stateless revise path. We never feed the raw transcript to revise — its
// conversational phrasing pulls the model out of clean JSON-edit mode. Always uses
// Haiku (mechanical extraction, not creative writing). Resolves to { summary, usage }.
async function distillContext(intake, { effort } = {}) {
  if (!Array.isArray(intake) || !intake.length) return { summary: '', usage: ZERO_USAGE };
  const prompt = `PLANNING CONVERSATION:\n${renderTranscript(intake)}`;
  const { data, usage } = await analyze(DISTILL_SYSTEM, prompt, { model: 'haiku', effort });
...
// A stateless JSON analysis pass for the learn pipeline's classifier. No tools, no
// voice; send a system prompt + data, get back a parsed JSON object. Defaults to
// Haiku (classification, not creative writing). One sterner retry; usage summed.
async function analyze(systemPrompt, prompt, { model = 'haiku', effort } = {}) {
  const sys = systemPrompt + NO_TOOLS_NOTE;
  const r = await runTurn(sys, prompt, { model, effort, web: false });
  let usage = r.usage;
  let data = firstJsonObject(r.text);
  if (!data) {
    const r2 = await runTurn(sys, prompt + '\n\nReply with ONLY the JSON object described — no prose, no code fence.', { model, effort, web: false });
    usage = sumUsage(usage, r2.usage);
    data = firstJsonObject(r2.text);
  }
  return { data, usage };
}
