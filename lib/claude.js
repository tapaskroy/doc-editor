...
function generate(premise, { onDelta, onDone, onError, onReset, model, effort, web, style, references, addDir, kind, intake, memory }) {
  ...
  // about the user, carrying the grounding/don't-volunteer guardrail. Lives in the
  // system prompt (like the voice guide), not the user prompt.
  const system =
    base + ANTI_TIC_NOTE + (web ? WEB_TOOLS_NOTE : NO_TOOLS_NOTE) + styleNote(style) + (memory || '') + (hasRefs ? ATTACH_NOTE : '');
...
async function revise({ markdown, comments = [], instruction = '', history = [], contextSummary = '', memory = '', model, effort, web, style, references, addDir }) {
  ...
  const system =
    REVISE_SYSTEM + ANTI_TIC_NOTE + (web ? WEB_TOOLS_NOTE : NO_TOOLS_NOTE) + styleNote(style) + (memory || '') + (hasRefs ? ATTACH_NOTE : '');
