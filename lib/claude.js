...
function generate(premise, { onDelta, onDone, onError, onReset, model, effort, web, style }) {
  const child = spawnClaude(
    [
      '--output-format', 'stream-json', '--include-partial-messages', '--verbose',
      '--system-prompt', GENERATE_SYSTEM + (web ? WEB_TOOLS_NOTE : NO_TOOLS_NOTE) + styleNote(style),
      ...toolArgs(web),
...
async function revise({ markdown, comments = [], instruction = '', history = [], model, effort, web, style }) {
  ...
  const system = REVISE_SYSTEM + (web ? WEB_TOOLS_NOTE : NO_TOOLS_NOTE) + styleNote(style);
