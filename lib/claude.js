...
function generate(premise, { onDelta, onDone, onError, onReset, model, effort, web, style, references, addDir, kind, intake }) {
  ...
  const system =
    base + ANTI_TIC_NOTE + (web ? WEB_TOOLS_NOTE : NO_TOOLS_NOTE) + styleNote(style) + (hasRefs ? ATTACH_NOTE : '');
...
async function revise({ markdown, comments = [], instruction = '', history = [], contextSummary = '', model, effort, web, style, references, addDir }) {
  ...
  const system =
    REVISE_SYSTEM + ANTI_TIC_NOTE + (web ? WEB_TOOLS_NOTE : NO_TOOLS_NOTE) + styleNote(style) + (hasRefs ? ATTACH_NOTE : '');
