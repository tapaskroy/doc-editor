...
function create(premise, { kind, email, voice, intake } = {}) {
  ensureDir();
  const id = newId();
  ...
  const meta = {
    ...
    voice: voice || null,
    // The raw "talk about it first" planning transcript, kept so generation can
    // ground on the user's exact words (verbatim) and never re-loses specifics the
    // compiled brief dropped. null when drafted from a bare premise.
    intake: Array.isArray(intake) && intake.length ? intake : null,
    // A distilled, fact-preserving summary of `intake` for the stateless revise
    // path (the raw transcript would break JSON-edit mode). Computed lazily on the
    // first revise; cleared on regenerate so it re-distills.
    contextSummary: null,
    // Per-document gate on the personal-memory leakage guardrail. When false
    // (default) memory only GROUNDS the writing (prevents fabrication); when true,
    // the writer MAY include known personal facts in the OUTPUT.
    usePersonalFacts: false,
    // When the intake transcript was mined for durable memory facts (once, after the
    // first draft). null until done; prevents re-capturing on every regenerate.
    capturedAt: null,
    createdAt: now,
    updatedAt: now,
  };
...
// Store the distilled context summary (from the intake transcript) used to ground
// revisions. Pass null/'' to clear it (e.g. on regenerate, to force a re-distill).
function setContextSummary(id, summary) {
  const meta = readMeta(id);
  meta.contextSummary = summary || null;
  meta.updatedAt = new Date().toISOString();
  fs.writeFileSync(metaPath(id), JSON.stringify(meta, null, 2));
  return meta;
}

// Stamp that this doc's intake has been mined for durable memory facts. Defaults to
// now; pass null to CLEAR it (so a failed capture becomes retryable on a later draft).
function setCapturedAt(id, when = new Date().toISOString()) {
  const meta = readMeta(id);
  meta.capturedAt = when || null;
  fs.writeFileSync(metaPath(id), JSON.stringify(meta, null, 2));
  return meta;
}

// Toggle whether this document may surface known personal facts in its OUTPUT
// (the per-doc half of the leakage guardrail). Grounding is always on regardless.
function setUsePersonalFacts(id, on) {
  const meta = readMeta(id);
  meta.usePersonalFacts = !!on;
  meta.updatedAt = new Date().toISOString();
  fs.writeFileSync(metaPath(id), JSON.stringify(meta, null, 2));
  return meta;
}
