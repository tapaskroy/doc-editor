...
async function captureFromIntake(intake, { model } = {}) {
  ...
  // `analyze` returns null ONLY when the model's output couldn't be parsed (a genuine
  // empty result is `{facts:[]}`). Throw so the caller can tell a failure apart from
  // "nothing durable to learn" — otherwise a transient hiccup looks like success and,
  // via the run-once guard, permanently suppresses capture for this doc.
  if (data == null) throw new Error('capture: model produced no parseable output');
...
function applyCandidate(docId, voiceId, candidate) {
  ...
  // Claude-corrections → the feedback channel (process feedback, never the voice).
  // These now also reach the prompt: feedback.compose() injects them as "avoid these".
  ...
  // Durable facts about the user/world → personal memory, kept immediately (the user
  // approved it through the gate), so they ground future writing via memory.compose().
  // No voice needed — memory is global. (Previously these went to the voice store as
  // 'context' rules, which compose() ignored, so keeping a context lesson did nothing.)
  if (candidate.target === 'context') {
    let title = 'a document';
    try { title = docs.readMeta(docId).title || title; } catch { /* doc may be gone */ }
    const [item] = memory.propose([{ text: candidate.text, topic: 'profile', section: 'other', source: 'edits', provenance: `Learned from your edits to "${title}"` }]);
    if (item) memory.keep(item.id);
    return { ok: true, where: 'memory', item: item || null };
  }

  // Style/taste preferences → the voice store as an active rule (injected by compose).
  if (candidate.target === 'voice') {
