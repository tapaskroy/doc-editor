...
// Tuned for relevance: translate the user's words into a focused provider query,
// scope to the inbox by default, and never pad the results to a count.
async function searchThreads(query, { limit = 10, model = 'sonnet' } = {}) {
  ...
  const system =
    `You search the user's mail with the ${tool} tool and return only the MOST RELEVANT recent threads.\n` +
    `Translate the request into a focused provider query: a person's name → from:/to: that person; keywords → match ` +
    `subject/body; default-scope to the inbox (Gmail: in:inbox) unless the user clearly wants archived or all mail. ` +
    `Do NOT pad — return only threads that genuinely match (fewer is better than irrelevant), newest first, at most ${limit}.\n` +
    `Return ONLY JSON (no prose, no code fence): ` +
    `{"threads":[{"id":"…","subject":"…","from":"…","snippet":"…","date":"…","unread":true|false}]}\n` +
    `Use only headers/snippets — do not open full bodies, and do not modify anything.`;
  const w = await runClaude({ system, prompt: `Find threads matching: ${query || 'recent inbox'}`, tools: [tool], model });
...
async function inbox({ limit = 10, model = 'sonnet' } = {}) {
  ...
  const system =
    `You list the user's most recent PRIMARY inbox threads using the ${tool} tool — important personal mail, ` +
    `EXCLUDING promotional / social / bulk / notification / spam categories where the provider distinguishes them. ` +
    `For Gmail, use the query "in:inbox category:primary". Return ONLY JSON (no prose, no code fence):\n` +
    `{"threads":[{"id":"…","subject":"…","from":"…","snippet":"…","date":"…","unread":true|false}]}\n` +
    `At most ${limit}, newest first. Use only headers/snippets; do not open bodies or modify anything.`;
  const w = await runClaude({ system, prompt: `List my ${limit} most recent Primary inbox threads.`, tools: [tool], model });
  const o = firstJsonObject(w.result);
  return (o && Array.isArray(o.threads)) ? o.threads : [];
}
