...
function deleteTopic(name) {
  try { fs.unlinkSync(topicPath(name)); return true; } catch { return false; }
}
...
// Remove the first bullet line matching `text` (used by forget). Returns whether a
// line was actually removed — the caller must NOT claim success if this is false.
...
// Normalized texts of the bullet lines already present in one Markdown file. The
// Markdown is authoritative for content, so capture/keep dedup against it (not just
// the manifest) — covering hand-edits and crash-recovery.
function fileBulletSet(file) {
  const set = new Set();
  let raw = '';
  try { raw = fs.readFileSync(file, 'utf8'); } catch { /* absent */ }
...
// The union of bullet texts across USER.md and every topic file.
function markdownBulletSet() {
  const set = fileBulletSet(PROFILE_PATH);
  for (const t of listTopics()) for (const b of fileBulletSet(topicPath(t))) set.add(b);
  return set;
}
...
// Markdown until kept). Dedups against every known manifest item (any status) AND
// against facts already in the canonical Markdown (the user may have typed them by
// hand), so the same fact isn't re-proposed after it was kept, discarded, or
// hand-written. Returns the items actually added.
function propose(items = []) {
  const data = loadManifest();
  const seen = new Set([...(data.items || []).map((i) => norm(i.text)), ...markdownBulletSet()]);
...
// Idempotent: if the fact already lives in the target Markdown (a crash between the
// two writes left the manifest behind, or the user typed it by hand), we skip the
// append rather than duplicate it — Markdown is authoritative.
function keep(id) {
  const data = loadManifest();
  const item = getItem(data, id);
  if (!item || item.status !== 'unsaved') return null;
  ensureDir();
  const isProfile = !item.topic || item.topic === 'profile';
  const file = isProfile ? PROFILE_PATH : topicPath(item.topic);
  if (!fileBulletSet(file).has(norm(item.text))) {
    if (isProfile) {
      appendBullet(file, SECTION_TITLES[item.section] || SECTION_TITLES.other, item.text);
    } else {
...
// Forget a kept fact: remove it from the Markdown AND tombstone it. Returns the item
// with `removed` = whether the Markdown line was actually deleted. When false (the
// bullet text drifted via a hand-edit), the fact may still be in USER.md — the caller
// must tell the user rather than silently implying it's gone.
function forget(id) {
  const data = loadManifest();
  const item = getItem(data, id);
  if (!item) return null;
  const file = (!item.topic || item.topic === 'profile') ? PROFILE_PATH : topicPath(item.topic);
  const removed = removeBullet(file, item.text);
  item.status = 'discarded';
  saveManifest(data);
  return { ...item, removed };
}
...
function syncToClaudeDir() {
  ...
  // We deliberately do NOT write an `@USER.md` import into ~/.claude/CLAUDE.md.
  // Auto-importing it would make `claude -p` auto-load the whole profile into
  // doc-editor's OWN writing calls (extra cost + a leakage-guardrail bypass), and the
  // `--setting-sources` flag that used to suppress that is incompatible with some
  // `claude` installs. A bare `~/.claude/USER.md` symlink is NOT auto-loaded, so it's
  // safe. To share the profile with their OTHER Claude sessions, the user adds the one
  // line below to ~/.claude/CLAUDE.md themselves (surfaced in the Profile tab).
  const alreadyImported = (() => {
    try { return /^\s*@USER\.md\s*$/m.test(fs.readFileSync(path.join(CLAUDE_DIR, 'CLAUDE.md'), 'utf8')); }
    catch { return false; }
  })();
  return { ok: true, link, linkMode, importLine: '@USER.md', alreadyImported };
}

module.exports = {
  // paths (handy for the server + tests)
  MEM_DIR, TOPICS_DIR, PROFILE_PATH, MANIFEST_PATH, CLAUDE_DIR, SECTION_TITLES, sanitizeTopic,
  ensureDir, storeExists,
  // markdown content
  readProfile, writeProfile, listTopics, readTopic, writeTopic, deleteTopic, topicPath,
  // manifest / queue
