...
// kind: 'ai' | 'manual' | 'restore'. `voice` records which voice produced an AI
// snapshot, so the learn pipeline attributes edits to the right voice (a lookup,
// not a guess). Returns the metadata list (no full text).
function add(id, { label, markdown, kind = 'ai', model = null, usd = 0, voice = null }) {
  ...
  if (kind === 'manual' && last && last.kind === 'manual' && Date.parse(now) - Date.parse(last.at) < COALESCE_MS) {
    ...
    arr.push({ vid: crypto.randomUUID().slice(0, 8), label, kind, model, usd, voice, at: now, markdown });
  }
  writeAll(id, arr);
  return list(id);
}
...
// A snapshot paired with the one immediately before it — for showing what THIS
// snapshot changed. Both are stored Markdown of the same lineage, so the diff is
// clean (no turndown re-serialization noise). before is '' for the first version.
function diffPair(id, vid) {
  const arr = readAll(id);
  const idx = arr.findIndex((v) => v.vid === vid);
  if (idx === -1) return null;
  return { label: arr[idx].label, before: idx > 0 ? arr[idx - 1].markdown : '', after: arr[idx].markdown };
}
...
module.exports = { add, list, get, previous, diffPair, remove, VERSIONS_DIR };
